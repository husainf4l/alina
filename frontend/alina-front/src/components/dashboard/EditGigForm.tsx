...
export default function EditGigForm({ gigId }: { gigId: string }) {
  ...
  // Load categories and gig data in parallel
  useEffect(() => {
    apiClient
  ...
  useEffect(() => {
    apiClient
      .get(`/api/Marketplace/gigs/${gigId}`)
      .then(({ data }) => {
        setForm({
          title: data.title ?? "",
  ...
  // Once both categories + gig are loaded, pre-select parent
  useEffect(() => {
    if (!form.categoryId || !categories.length) return;
    // Check if categoryId is a subcategory of some parent
    for (const parent of categories) {
      if (parent.id === form.categoryId) {
        setSelectedParentId(parent.id);
        return;
      }
      if (parent.subCategories?.some((s) => s.id === form.categoryId)) {
        setSelectedParentId(parent.id);
        return;
      }
    }
  ...
  const handleMediaUpload = useCallback(async (file: File, type: "main" | "gallery"): Promise<void> => {
    ...
    try {
      ...
      if (type === "main") setMainImagePreview(form.mainImage);
  ...
  const canProceed = () => {
    if (step === "basics") return form.title.trim().length >= 10 && form.categoryId !== "";
    if (step === "details") return form.description.trim().length >= 30 && Number(form.startingPrice) > 0 && Number(form.deliveryTimeInDays) >= 1;
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await apiClient.put(`/api/Marketplace/gigs/${gigId}`, {
  ...
  if (user && !isSeller) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-16 text-center">
        ...
        <Button onClick={() => router.push("/dashboard/profile")}>{t("notSellerCta")}</Button>
      </div>
    );
  }
  ...
  if (loadingGig) {
    return (
      <div className="flex items-center justify-center py-24">
        ...
        <span className="ml-3 text-sm text-muted-foreground">{tGigs("loadingGig")}</span>
      </div>
    );
  }
                ...
                >
                  ...
                  <div className="text-left hidden sm:block">
                    ...
                    <p className="text-[10px] text-muted-foreground leading-tight">
                      ...
                      {s === "media" && (mainImagePreview ? "Image added ✓" : "Photos")}
                    </p>
                  </div>
                </button>
                {i < STEPS.length - 1 && (
          ...
          <div className="rounded-2xl border border-border bg-card p-8">
                ...
                <Field label={t("categoryLabel")}>
                  {loadingCategories ? (
                    <div className="flex items-center gap-2 h-11 rounded-xl border border-border bg-muted/40 px-3 text-sm text-muted-foreground">
                      <Loader2 className="size-4 animate-spin" />{t("loadingCategories")}
                    </div>
                  ) : (
                    <select
              ...
              <div className="flex flex-col gap-7">
                ...
                <Field label={t("descriptionLabel")}>
                  ...
                  <div className="mt-1.5 flex items-center justify-between">
                    <div className="flex-1 h-1 rounded-full bg-muted overflow-hidden mr-3">
                      <div className="h-full rounded-full transition-all duration-300 bg-[#B05088]" style={{ width: `${Math.min((form.description.length / 5000) * 100, 100)}%` }} />
                    </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label={t("priceLabel")}>
                    <div className="relative">
                      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground"><DollarSign className="size-4" /></span>
                      <input type="number" min={1} step={0.01} value={form.startingPrice} onChange={(e) => set("startingPrice", e.target.value)}
                        placeholder="5.00" className={cn(inputCls, "pl-9")} />
                    </div>
                  </Field>
                  <Field label={t("deliveryLabel")}>
                    <div className="flex flex-col gap-2">
                      <div className="relative">
                        <input type="number" min={1} max={365} value={form.deliveryTimeInDays} onChange={(e) => set("deliveryTimeInDays", e.target.value)}
                          className={cn(inputCls, "pr-12")} />
                        <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground">{t("days")}</span>
                      </div>
                      <div className="flex gap-1.5 flex-wrap">
                        {[1, 3, 7, 14, 30].map((d) => (
                          <button key={d} type="button" onClick={() => set("deliveryTimeInDays", String(d))}
                            className={cn("rounded-lg px-2.5 py-1 text-xs font-medium transition-colors border",
                              form.deliveryTimeInDays === String(d)
                                ...
                                : "border-border text-muted-foreground hover:border-[#B05088] hover:text-[#B05088]"
                            )}>{d}d</button>
                        ))}
                      </div>
                    </div>
            ...
            {step === "media" && (
              <div className="flex flex-col gap-7">
                ...
                <Field label={t("mainImageLabel")}>
                  <input ref={mainImageRef} type="file" accept="image/jpeg,image/png,image/gif,image/webp" className="hidden"
                    onChange={(e) => { const f = e.target.files?.[0]; if (f) handleMediaUpload(f, "main"); e.target.value = ""; }} />
                  {(mainImagePreview || form.mainImage) ? (
                    <div className="group relative rounded-2xl overflow-hidden border border-border bg-muted">
                      ...
                      <img src={mainImagePreview || form.mainImage} alt="Main image" className="h-56 w-full object-cover" />
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/50 transition-colors flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100">
                        ...
                        <button type="button" onClick={() => { setMainImagePreview(""); set("mainImage", ""); }}
                          className="flex items-center gap-1.5 rounded-xl bg-white/20 backdrop-blur-sm border border-white/30 px-4 py-2 text-white text-sm font-medium hover:bg-white/30 transition-colors">
                    ...
                    <button type="button" onClick={() => mainImageRef.current?.click()} disabled={uploadingMainImage}
                      className="group relative flex flex-col items-center justify-center gap-3 h-56 w-full rounded-2xl border-2 border-dashed border-border hover:border-[#B05088] hover:bg-[#B05088]/5 transition-all text-muted-foreground disabled:opacity-50">
                      {uploadingMainImage ? (
                        <><Loader2 className="size-10 animate-spin text-[#B05088]" /><span className="text-sm font-medium">{t("uploadingImage")}</span></>
                      ) : (
                        <>
                ...
                <Field label={t("galleryLabel")} hint={`${Math.max(galleryPreviews.length, form.galleryImages.length)}/5 · ${t("optional")}`}>
                  <input ref={galleryRef} type="file" accept="image/jpeg,image/png,image/gif,image/webp" multiple className="hidden"
                    onChange={(e) => {
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    ...
                    {Math.max(galleryPreviews.length, form.galleryImages.length) < 5 && (
                      <button type="button" onClick={() => galleryRef.current?.click()} disabled={uploadingGallery}
                        className="group aspect-square rounded-xl border-2 border-dashed border-border hover:border-[#B05088] hover:bg-[#B05088]/5 transition-all flex flex-col items-center justify-center gap-1.5 text-muted-foreground disabled:opacity-50">
                        {uploadingGallery
                          ? <Loader2 className="size-6 animate-spin text-[#B05088]" />
                          : <><ImagePlus className="size-6 group-hover:text-[#B05088] transition-colors" /><span className="text-[10px] font-medium group-hover:text-[#B05088] transition-colors">{t("addImage")}</span></>
                        }
                      </button>
                    )}
                  </div>
            ...
            <div className="mt-8 flex items-center justify-between gap-4 border-t border-border pt-6">
              <div className="flex gap-1.5">
                {STEPS.map((_, i) => (
                  <div key={i} className={cn("h-1.5 rounded-full transition-all", i === stepIndex ? "w-6 bg-[#B05088]" : i < stepIndex ? "w-3 bg-[#B05088]/40" : "w-3 bg-muted")} />
                ))}
              </div>
              <div className="flex gap-2">
                {stepIndex > 0 && (
                  <Button variant="outline" onClick={prevStep} className="rounded-xl gap-1.5">
                    <ChevronLeft className="size-4" /> {t("back")}
                  </Button>
                )}
                {stepIndex < STEPS.length - 1 ? (
                  <Button onClick={nextStep} disabled={!canProceed()} className="rounded-xl gap-1.5 bg-[#B05088] hover:bg-[#B05088]/90 text-white">
                    {t("next")} <ChevronRight className="size-4" />
                  </Button>
                ) : (
                  <Button onClick={handleSubmit} disabled={submitting} className="rounded-xl gap-1.5 min-w-28 bg-[#B05088] hover:bg-[#B05088]/90 text-white">
                    {submitting ? <><Loader2 className="size-4 animate-spin" /> {tGigs("saving")}</> : tGigs("saveButton")}
                  </Button>
                )}
              </div>
            </div>
          ...
          <div className="sticky top-6 flex flex-col gap-4">
            ...
            <div className="rounded-2xl border border-border bg-card overflow-hidden shadow-sm">
              <div className="relative h-44 bg-gradient-to-br from-muted to-muted/60 flex items-center justify-center overflow-hidden">
                {(mainImagePreview || form.mainImage) ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={mainImagePreview || form.mainImage} alt="Preview" className="h-full w-full object-cover" />
                ) : (
              ...
              <div className="p-4 flex flex-col gap-3">
                <p className="text-sm font-semibold text-foreground line-clamp-2 leading-snug">
                  {form.title || <span className="text-muted-foreground/50 italic">Your gig title…</span>}
                </p>
                <div className="flex items-center gap-1.5">
                  <div className="size-5 rounded-full bg-muted" />
                  <span className="text-xs text-muted-foreground">{user?.displayName ?? user?.fullName ?? "You"}</span>
                </div>
                <div className="flex items-center gap-1">
                  {[...Array(5)].map((_, i) => <Star key={i} className="size-3 text-muted-foreground/30" />)}
                </div>
            ...
            <div className="rounded-2xl border border-border bg-card p-4 flex flex-col gap-2.5">
              <p className="text-xs font-semibold text-foreground">Gig Checklist</p>
              {[
                { label: "Title (10+ chars)", done: form.title.length >= 10 },
                { label: "Category selected", done: !!form.categoryId },
                { label: "Description (30+ chars)", done: form.description.length >= 30 },
                { label: "Price set", done: Number(form.startingPrice) > 0 },
                { label: "Main image", done: !!(mainImagePreview || form.mainImage) },
              ].map(({ label, done }) => (
                <div key={label} className="flex items-center gap-2">
                  <div className={cn("flex size-4 shrink-0 items-center justify-center rounded-full", done ? "bg-[#3E9666]" : "bg-muted")}>
