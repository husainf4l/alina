...
public class ChatHub : Hub
{
    ...
    public async Task SendMessage(Guid receiverId, string content, string? attachmentUrl = null)
    {
        ...
        // Upsert Conversation record (keyed on the canonical pair of User GUIDs)
        var senderUserId = senderId.Value;
        var receiverUserId = receiverProfile.UserId;

        var conversation = await _context.Conversations.FirstOrDefaultAsync(c =>
            (c.User1Id == senderUserId && c.User2Id == receiverUserId) ||
            (c.User1Id == receiverUserId && c.User2Id == senderUserId));
        ...
        conversation.LastMessageText = content?.Length > 100 ? content[..100] + "…" : content ?? "";
        conversation.LastMessageAt = DateTime.UtcNow;
        conversation.LastMessageSenderId = senderProfile.UserId;
        conversation.UpdatedAt = DateTime.UtcNow;

        // Increment unread count for the recipient
        if (conversation.User1Id == receiverUserId)
