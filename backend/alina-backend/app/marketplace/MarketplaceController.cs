...
public class MarketplaceController : ControllerBase
{
    ...
    public MarketplaceController(AppDbContext context, alina_backend.app.finance.ICurrencyService currencyService)
    {
        _context = context;
    ...
    public async Task<IActionResult> RecordSearchAnalytics([FromBody] SearchAnalyticsDto dto)
    {
        ...
        // Try to find existing analytics for this term
        var existingAnalytics = await _context.SearchAnalytics
            .FirstOrDefaultAsync(sa => sa.SearchTerm.ToLower() == dto.SearchTerm.ToLower());

        if (existingAnalytics != null)
        {
            // Update existing record
            existingAnalytics.SearchCount++;
            existingAnalytics.LastSearchedAt = DateTime.UtcNow;
        }
        else
        {
            // Create new record
