...
public class MarketplaceOpsController : ControllerBase
{
    ...
    private readonly alina_backend.app.finance.ICurrencyService _currencyService;
    ...
    public MarketplaceOpsController(AppDbContext context, alina_backend.app.finance.ICurrencyService currencyService, ILogger<MarketplaceOpsController> logger)
    {
        _context = context;
    ...
    public async Task<IActionResult> UpdateOrderStatus(Guid id, UpdateOrderStatusDto dto)
    {
        ...
        if (dto.Status == OrderStatus.Cancelled && oldStatus != OrderStatus.Cancelled)
        {
            ...
            try
            {
                ...
                // Atomic refund: deduct from escrow, add to available
                buyerWallet.EscrowBalance -= order.Amount;
    ...
    public async Task<ActionResult<ReviewDto>> CreateReview(CreateReviewDto dto)
    {
        ...
        // Update Gig rating only if it's a Gig order
        if (order.GigId.HasValue)
        {
            var gig = order.Gig;
            var allGigRatings = await _context.Reviews.Where(r => r.GigId == gig.Id).Select(r => r.Rating).ToListAsync();
            allGigRatings.Add(dto.Rating);
            gig.AverageRating = allGigRatings.Average();
            gig.ReviewCount = allGigRatings.Count;
        }
    ...
    public async Task<IActionResult> CompleteOrder(Guid id)
    {
        ...
        var sellerProfile = await _context.Profiles.FirstOrDefaultAsync(p => p.UserId == userId);
        if (sellerProfile == null) return BadRequest("Profile not found");
